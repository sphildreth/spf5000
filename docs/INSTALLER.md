...
The installer uses `DECENTDB_RELEASE_TAG=latest` by default. Set `DECENTDB_RELEASE_TAG=vX.Y.Z` in the environment if you want to override that latest-release behavior. For SPF5000, use DecentDB `v2.0.0` or newer for the latest features and improvements.
