...
from app.schemas.auth import LoginRequest, SessionResponse, SessionUserResponse, SetupRequest
...
def login(request: Request, body: LoginRequest) -> SessionResponse:
    ...
    if not _service.is_bootstrapped():
        raise HTTPException(status_code=409, detail="System setup must be completed before login")
