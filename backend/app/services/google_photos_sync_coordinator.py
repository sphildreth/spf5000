...
class GooglePhotosSyncCoordinator:
    ...
    def start(self) -> None:
        if self._thread is not None:
            return
        self._thread = threading.Thread(target=self._run, name="google-photos-sync", daemon=True)
        self._thread.start()
    ...
    def request_sync(self, trigger: str = "manual") -> tuple[bool, bool]:
        with self._lock:
            already_queued = trigger in self._queued_triggers or (self._is_running and self._last_trigger == trigger)
    ...
    def _run(self) -> None:
        next_periodic_deadline = time.monotonic() + max(1, settings.google_photos_sync_cadence_seconds)
        while not self._stop_event.is_set():
            ...
            try:
                ...
                LOGGER.exception("Google Photos sync trigger %s failed", trigger)
            finally:
                self._is_running = False
                next_periodic_deadline = time.monotonic() + max(1, settings.google_photos_sync_cadence_seconds)
