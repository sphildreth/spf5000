...
import { asArray, asBoolean, asOptionalNumber, asOptionalString, asRecord, asString, asStringArray, type CollectionSummary, type CollectionUpsertRequest } from './types'
...
export async function updateCollection(id: string, request: CollectionUpsertRequest): Promise<CollectionSummary> {
...
function normalizeCollection(item: unknown, index: number): CollectionSummary {
  ...
  const sourceId = asOptionalString(record?.source_id)
