...
export async function startGooglePhotosConnect(): Promise<GooglePhotosProviderStatus> {
  const payload = await apiPost<Record<string, never>, unknown>('/api/google-photos/connect/start', {})
...
export async function pollGooglePhotosConnect(): Promise<GooglePhotosProviderStatus> {
  const payload = await apiPost<Record<string, never>, unknown>('/api/google-photos/connect/poll', {})
  return payload === undefined ? getGooglePhotosStatus() : normalizeGooglePhotosStatus(payload)
}
...
function normalizeGooglePhotosStatus(value: unknown): GooglePhotosProviderStatus {
  ...
  const statusRecord = asRecord(getValue(record, 'status')) ?? record
  const providerRecord = asRecord(getValue(statusRecord, 'provider'))
  const pendingAuthRecord = asRecord(
    getValue(statusRecord, 'pending_auth', 'pendingAuth', 'device_auth_flow', 'deviceAuthFlow', 'auth_flow', 'authFlow'),
  )
  const accountRecord = asRecord(getValue(statusRecord, 'account', 'linked_account', 'linkedAccount'))
  const deviceRecord = asRecord(getValue(statusRecord, 'device', 'ambient_device', 'ambientDevice'))
  const syncRecord = asRecord(
    getValue(statusRecord, 'latest_sync_run', 'latestSyncRun', 'latest_sync', 'latestSync', 'sync_run', 'syncRun'),
  )
  const warnings = asArray(getValue(statusRecord, 'warnings'), (item) => asString(item, ''))
  ...
  return {
    provider_available: asBoolean(
      getValue(statusRecord, 'provider_available', 'providerAvailable', 'available') ??
        getValue(providerRecord, 'available'),
      false,
    ),
    provider_configured: asBoolean(
      getValue(statusRecord, 'provider_configured', 'providerConfigured', 'configured') ??
        getValue(providerRecord, 'configured'),
      false,
    ),
    connection_state: asString(
      getValue(statusRecord, 'connection_state', 'connectionState') ??
    ...
    device: deviceRecord ? normalizeDevice({ ...deviceRecord, selected_media_sources: deviceMediaSources }) : null,
    latest_sync_run: syncRecord ? normalizeSyncRun(syncRecord) : null,
    cached_asset_count: asNumber(getValue(statusRecord, 'cached_asset_count', 'cachedAssetCount', 'asset_count'), 0),
    last_successful_sync_at:
      asOptionalString(getValue(statusRecord, 'last_successful_sync_at', 'lastSuccessfulSyncAt')) ?? null,
    warning:
      asOptionalString(getValue(statusRecord, 'warning', 'sync_warning', 'syncWarning')) ??
      warnings.find((item) => item.length > 0) ??
      null,
    error:
...
function normalizeSyncRun(record: JsonRecord): GooglePhotosSyncRunSummary {
  return {
    ...
    updated_count: asNumber(getValue(record, 'updated_count', 'updatedCount', 'duplicate_count', 'duplicateCount'), 0),
